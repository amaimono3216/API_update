 ...
 */
export async function judgeImpact(candidates: ImpactCandidate[], log: Logger): Promise<ImpactJudgement[]> {
  ...
  for (let offset = 0; offset < candidates.length; offset += BATCH_SIZE) {
    ...
    judgements.push(...(await judgeBatch(client, batch, log)));
  }
  return judgements;
}
...
async function judgeBatch(
  ...
  const response = await client.beta.messages.create({
    ...
    messages: [{ role: 'user', content: buildPrompt(batch, idOf) }],
  });
...
function buildPrompt(batch: ImpactCandidate[], idOf: (index: number) => string): string {
  const sections = batch.map((candidate, index) => {
